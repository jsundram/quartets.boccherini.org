...
When someone pastes the URL, iMessage/WhatsApp/Slack fetch og:image — and quietly SKIP one over
~300 KB, so a card that rasterized fine but never got compressed previews as *nothing*. make-og.sh
compresses + gates at generation time; this guards the commit, catching a hand-exported or
externally-produced PNG that never went through the script.

MAX_BYTES keeps a margin under WhatsApp's ~300 KB scrape cutoff (keep in sync with make-og.sh).

By hand:  python3 scripts/og-lint.py
"""
